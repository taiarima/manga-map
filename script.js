    ...
    mangaList.forEach(
      ({ coords, image, title, author, year, locationName, summary }) => {
        L.marker(coords)
          .addTo(this.#map)
          .bindPopup(
            `<div class="popup"><img class="manga-cover" src="${image}"/><span>Title: ${title}</span><span>Author: ${author}</span><span>${year}</span><span>Setting: ${locationName}</span><span>${summary}</span></div>`,
            { minWidth: 200 }
          );
      }
    );
